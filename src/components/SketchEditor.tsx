...
/** Create the appropriate fill style for a shape stroke */
const createShapeFillStyle = (
  ctx: CanvasRenderingContext2D,
  stroke: Stroke,
  x: number, y: number, w: number, h: number
): CanvasGradient | CanvasPattern | string | null => {
  if (!stroke.fillColor || !stroke.fillOpacity || stroke.fillOpacity <= 0) return null;
  const fillType = stroke.fillType || 'solid';
  const c1 = hexToRgba(stroke.fillColor, stroke.fillOpacity);
  const c2 = stroke.fillColor2 ? hexToRgba(stroke.fillColor2, stroke.fillOpacity) : hexToRgba(stroke.fillColor, stroke.fillOpacity * 0.2);
  const angle = (stroke.fillAngle ?? 135) * Math.PI / 180;
  ...
  switch (fillType) {
    ...
    case 'crosshatch': {
      ...
      const patSize = fillType === 'dots' ? 12 : 10;
      ...
      // Parse fill color for pattern
      const r2 = parseInt(stroke.fillColor.slice(1, 3), 16);
      const g2 = parseInt(stroke.fillColor.slice(3, 5), 16);
      const b2 = parseInt(stroke.fillColor.slice(5, 7), 16);
      pCtx.strokeStyle = `rgba(${r2},${g2},${b2},${stroke.fillOpacity})`;
      pCtx.fillStyle = `rgba(${r2},${g2},${b2},${stroke.fillOpacity})`;
      pCtx.lineWidth = 1.5;
      if (fillType === 'stripes') {
        pCtx.beginPath();
        pCtx.moveTo(0, patSize); pCtx.lineTo(patSize, 0);
        pCtx.stroke();
      } else if (fillType === 'dots') {
        pCtx.beginPath();
        pCtx.arc(patSize / 2, patSize / 2, 2, 0, Math.PI * 2);
        pCtx.fill();
      } else {
        pCtx.beginPath();
        pCtx.moveTo(0, patSize); pCtx.lineTo(patSize, 0);
        pCtx.moveTo(0, 0); pCtx.lineTo(patSize, patSize);
        pCtx.stroke();
      }
